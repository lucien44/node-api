...
export default class moyenneEcController {
    /**
    ...
    static create(req: Request, resp: Response) {
        ...
        MoyenneEc.save((err: any) => {
            if (err) resp.status(500).send(err);
            else resp.send(moyenneEc);
        })

    }

    static getAllMoyenneEc(req: Request, resp: Response) {
        moyenneEc.find((err: any, moyenneEc: any) => {
            if (err) {
    ...
    static getMoyenneEcById(req: Request, resp: Response) {

        moyenneEc.findById(req.params.id, (err: any, moyenneEc: any) => {
            if (err) { resp.status(500).send(err); }
            else { resp.send(moyenneEc); }
        });
    }
    ...
    static updateMoyenneEcById(req: Request, resp: Response) {
        moyenneEc.findByIdAndUpdate(req.params.id, req.body, (err: any, moyenneEc: any) => {
            if (err) resp.status(500).send(err);
            else {
    ...
    static deleteMoyenneEcById(req: Request, resp: Response) {
        moyenneEc.deleteOne({ _id: req.params.id }, (err: any) => {
            if (err) resp.status(500).send(err);
            else resp.send("Successfuly deleted MoyenneEc");
        });
    }
