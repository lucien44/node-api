...
const niveau_1 = __importDefault(require("../entity/niveau"));
class niveauController {
    ...
    static create(req, resp) {
        let Niveau = new niveau_1.default({
            nom: req.body.nom,
        ...
        Niveau.save((err) => {
            if (err)
                ...
                resp.send(niveau_1.default);
        });
    }
    static getAllNiveau(req, resp) {
        niveau_1.default.find((err, niveau) => {
            if (err) {
                resp.status(500).send(err);
            }
            else {
    ...
    static getNiveauById(req, resp) {
        niveau_1.default.findById(req.params.id, (err, niveau) => {
            if (err) {
                resp.status(500).send(err);
            }
            else {
    ...
    static updateNiveauById(req, resp) {
        niveau_1.default.findByIdAndUpdate(req.params.id, req.body, (err, niveau) => {
            if (err)
                resp.status(500).send(err);
            else {
    ...
    static deleteNiveauById(req, resp) {
        niveau_1.default.deleteOne({ _id: req.params.id }, (err) => {
            if (err)
                resp.status(500).send(err);
            else
                resp.send("Successfuly deleted Niveau");
        });
    }
}
exports.default = niveauController;
