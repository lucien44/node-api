...
class noteController {
    ...
    static create(req, resp) {
        ...
        Note.save((err) => {
            if (err)
                ...
                resp.send(note_1.default);
        });
    }
    static getAllNote(req, resp) {
        note_1.default.find((err, note) => {
            if (err) {
                resp.status(500).send(err);
            }
            else {
    ...
    static getNoteById(req, resp) {
        note_1.default.findById(req.params.id, (err, note) => {
            if (err) {
                resp.status(500).send(err);
            }
            else {
    ...
    static updateNoteById(req, resp) {
        note_1.default.findByIdAndUpdate(req.params.id, req.body, (err, note) => {
            if (err)
                resp.status(500).send(err);
            else {
    ...
    static deleteNoteById(req, resp) {
        note_1.default.deleteOne({ _id: req.params.id }, (err) => {
            if (err)
                resp.status(500).send(err);
            else
                resp.send("Successfuly deleted Note");
        });
    }
}
